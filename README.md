...
## Survey + PostHog Environment

Set these Convex environment variables for server-side analytics outbox delivery:

```bash
npx convex env set POSTHOG_API_KEY <your_posthog_project_api_key>
npx convex env set POSTHOG_HOST https://us.i.posthog.com
```

Set these frontend variables in `.env.local` for browser analytics:
